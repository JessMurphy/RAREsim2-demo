...
for(i in start:end){
  ...
  leg = read.table(paste0("./datasets/Hapgen", Nsim/1000, "K_pruned/", Pop, "/Round1/chr19.block37.", Pop, ".sim", i, ".", Nsim, ".", p.opp1[j], "fun2.100syn.protected.legend"), header=T) %>%
  ...
  cases.hap = fread(paste0("./datasets/Cases/", Pop, "/Round1/chr19.block37.", Pop, ".sim", i, ".", Ncase, ".power.cases.opp.", p.opp1[j], "fun2.100syn.haps-sample.gz"))
  cases.hap = as.data.frame(cases.hap)
  
  controls.hap = fread(paste0("./datasets/Controls/", Pop, "/Round1/chr19.block37.", Pop, ".sim", i, ".", Ncase, ".controls.opp.", p.opp2[j], "fun2.100syn.haps-remainder.gz"))
...
write.table(out.genes.all.p, paste0("./results/", Pop, "/power_opp_pvalues_", Pop, "_", p.opp1[j], "_", p.opp2[j], "_", end, ".txt"), quote=F, row.names=F, col.names=T)
...
write.table(power.genes.out, paste0("./results/", Pop, "/power_opp_results_", Pop, "_", p.opp1[j], "_", p.opp2[j], "_", end, ".txt"), quote=F, row.names=F, col.names=T)
