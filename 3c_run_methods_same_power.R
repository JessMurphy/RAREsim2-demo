...
p.case = c(160, 140, 120)                # percentages for the expected number of functional variants in cases
...
for(i in start:end){
  ...
  leg = read.table(paste0("./datasets/Hapgen", Nsim/1000, "K_pruned/", Pop, "/Round1/chr19.block37.", Pop, ".sim", i, ".", Nsim, ".", p.case[1], "fun.100syn.legend"), header=T) %>%
    mutate(gene=ifelse(gene=="ZNF333;ZNF333(NM_001352243:exon9:UTR5)", "ZNF333", gene))
  leg$row = 1:nrow(leg)
  
  # read in the haplotype files
  cases.hap = fread(paste0("./datasets/Cases/", Pop, "/Round1/chr19.block37.", Pop, ".sim", i, ".", Ncase, ".power.cases.same.", p.case[j], "fun.100syn.haps-sample.gz"))
  cases.hap = as.data.frame(cases.hap)
  
  controls.hap = fread(paste0("./datasets/Controls/", Pop, "/Round1/chr19.block37.", Pop, ".sim", i, ".", Ncase, ".controls.same.100fun.100syn.haps-remainder.gz"))
...
write.table(out.genes.all.p, paste0("./results/", Pop, "/power_same_pvalues_", Pop, "_", p.case[j], "_", end, ".txt"), quote=F, row.names=F, col.names=T)
...
write.table(power.genes.out, paste0("./results/", Pop, "/power_same_results_", Pop, "_", p.case[j], "_", end, ".txt"), quote=F, row.names=F, col.names=T)
